...
class TestNGTest extends TestFrameworkTest {
  ...
  def "test failed with success percentage generates spans"() {
    ...
    assertTraces(5) {
      trace(0, 1) {
        testSpan(it, 0, "org.example.TestFailedWithSuccessPercentage", "test_failed_with_success_percentage", TestDecorator.TEST_FAIL, exception)
      }
      trace(1, 1) {
        testSpan(it, 0, "org.example.TestFailedWithSuccessPercentage", "test_failed_with_success_percentage", TestDecorator.TEST_FAIL, exception)
      }
      trace(2, 1) {
        testSpan(it, 0, "org.example.TestFailedWithSuccessPercentage", "test_failed_with_success_percentage", TestDecorator.TEST_PASS, null)
      }
      trace(3, 1) {
        testSpan(it, 0, "org.example.TestFailedWithSuccessPercentage", "test_failed_with_success_percentage", TestDecorator.TEST_PASS, null)
      }
